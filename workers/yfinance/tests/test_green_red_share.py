...
`green_red_volume_ratio_1m`. It measures what share of total volume
during the trailing 30-day window occurred on up-bars (vs down-bars).
Range: 0..1. NaN when the window has no up or no down bars.
...
def test_all_green_share_is_one():
    # 5 up-days, each close > open.  share = sum_green / (sum_green + 0) = 1.0
    bars = _bars([
        (100, 110, 1000, 0),
        (100, 105,  500, 0),
        (100, 108,  700, 0),
...
def test_share_weights_high_volume_days_more_than_ratio():
    ...
    s = _green_red_volume_share_1m(bars["open"], bars["close"], bars["volume"], window=3)
    assert math.isclose(float(s.iloc[-1]), 200 / 1100, abs_tol=1e-9)
    # Sanity: share is small but non-zero — differs from a pure avg-based
    # metric that would also be 0.111 but for a different reason.
...
def test_rolling_window_advances_correctly():
    ...
    s = _green_red_volume_share_1m(bars["open"], bars["close"], bars["volume"], window=3)
    # last window: green=200 (100+100), red=200 → share = 0.5
