...
`green_red_volume_ratio_1m`. It returns a LINEAR signed net lean in [-1, 1]:

        net = (sum(volume on up-bars) − sum(volume on down-bars))
              / (sum on up + sum on down)

The sign carries the colour zone (> 0 = green-leaning, < 0 = red-leaning)
and the magnitude carries how far from balanced (0 = balanced, ±1 =
one-sided window). Default window is 21 trading days to match the
stocks page Range picker "1M" so the screener and the stocks-page pill
agree. NaN when the window has no up or no down bars.
...
def test_all_green_share_is_one():
    # 5 up-days, each close > open. net = (sum − 0) / sum = +1.0
    bars = _bars([
        (100, 110, 1000, 0),
        (100, 105,  500, 0),
        (100, 108,  700, 0),
...
def test_share_weights_high_volume_days_more_than_ratio():
    ...
    s = _green_red_volume_share_1m(bars["open"], bars["close"], bars["volume"], window=3)
    assert math.isclose(float(s.iloc[-1]), -700 / 1100, abs_tol=1e-9)
...
def test_rolling_window_advances_correctly():
    ...
    s = _green_red_volume_share_1m(bars["open"], bars["close"], bars["volume"], window=3)
    # last window: green=200 (100+100), red=200 → net = 0.0 (balanced).
